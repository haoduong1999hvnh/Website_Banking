...
<?php 
  ...
  if(isset($_POST['taikhoan'])){ 
    ...
    if ($taikhoan == "" || $matkhau == "") {
      ...
      $sql = "SELECT * FROM `tbl_nguoidung` as t1 JOIN tbl_thanhvien as t2 ON t1.mathanhvien  = t2.id_thanhvien WHERE t1.username = '$taikhoan' AND t1.password = '$matkhau'";
      $query = $con->query($sql);
      if(mysqli_num_rows($query = $con->query($sql)) > 0){
        $row = $query->fetch_assoc();
        $_SESSION['tk']['id'] = $row['id'];
        $_SESSION['tk']['id_thanhvien'] = $row['id_thanhvien'];
			...
			<div class="wrap-login100">
				...
				Đăng nhập
					...
					<div class="wrap-input100 rs1-wrap-input100 validate-input m-b-20" data-validate="Type user name">
						<input id="first-name" class="input100" type="text" name="taikhoan"  placeholder="User name">
